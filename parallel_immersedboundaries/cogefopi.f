...
c      n1m=n1-1!PARALL
      n2m=n2-1
      n3m=n3-1
c      if(n1.eq.1) n1m = 1   !PARALL
      if(n1mglob.eq.1) then   !PARALL
        iaxsy=0
      else
        iaxsy=1
      endif
...
c
      read(60,*) npunz_i
      if (myid.eq.0)write(6,*) ' Number of internal points for q3 = '
     $     ,npunz_i
      if(npunz_i.gt.mpun) 
...
c    %            ,n=1,npunz_i)
      ...
      read(60,*) npunz_e
      alx3=x3c(nz)-x3c(1)
      if (myid.eq.0)write(6,*) ' Number of external points for q3 = '
     $     ,npunz_e
      if(npunz_e.gt.mpun) 
     %  write(*,*) ' WARNING: dimension mpun too small for q3',mpun
...
c
      read(60,*) npunr_i
      if (myid.eq.0)write(6,*) ' Number of internal points for q2 = '
     $     ,npunr_i
      if(npunr_i.gt.mpun) 
...
c    %            ,n=1,npunr_i)
      ...
      enddo
      read(60,*) npunr_e
      if (myid.eq.0)write(6,*) ' Number of external points for q2 = '
     $     ,npunr_e
      if(npunr_e.gt.mpun) 
     %  write(*,*) ' WARNING: dimension mpun too small for q2',mpun
...
c
      read(60,*) npunt_i
      if (myid.eq.0)write(6,*) ' Number of internal points for q1 = '
     $     ,npunt_i
      if(npunt_i.gt.mpun) 
...
c    %            ,n=1,npunt_i)
      ...
      enddo
      read(60,*) npunt_e
      if (myid.eq.0)write(6,*) ' Number of external points for q1 = '
     $     ,npunt_e
      if(npunt_e.gt.mpun) 
     %  write(*,*) ' WARNING: dimension mpun too small for q1',mpun
