...
!$OMP$ PRIVATE(xr,work,is,ip,id)
        do k=1,n3m
          ...
          if (k.eq.n3/2)write(41,*)myid,((xr(i,j),i=2,n1),j=1,n2m)
          call fft99(xr,work,trigx1,ifx1,1,m1+1,n1m,n2m,-1)
          if (k.eq.n3/2)write(61,*)myid,((xr(i,j),i=1,n1+1),j=1,n2m)
          do j=1,n2m
            do i=1,n1mh
              ip=2*i
              id=2*i-1
...
C     
       if (i.eq.1)then
          open(99,file='real-in.dat',form='formatted',status='unknown')
     ...
     $         1,', F=POINT'
          ...
          do j = 1,n2
             do k = 1,n3
                write(99,*)x3c(k),x2c(j),x1c(ii),yfisl(k,j)
             enddo
          enddo
          close(99)
          open(99,file='coeff-k.dat',form='formatted',status='unknown')
          write(99,*)(x3c(k),amkl(k),ackl(k),apkl(k),k=1,n3)
          close(99)
          open(99,file='coeff-j.dat',form='formatted',status='unknown')
          write(99,*)(x2c(j),amphj(j),acphj(j),apphj(j),j=1,n2)
          close(99)
        endif
C     
...
c    %             ,1,n3m,amkl,ackl,apkl,m3,yfisl,ierror,wl1)
        ...
        if (i.eq.1)then
          open(99,file='real-out.dat',form='formatted',status='unknown')
          write(99,*) 'VARIABLES = "X","Y","Z","qcap"'
           write(99,*) 'ZONE I = ',n3,', J = ',n2,', K = ',
     $         1,', F=POINT'
           ii = 1
...
!$OMP$ PRIVATE(xr,work,is,ip,id)
        do k=1,n3m
          do j=1,n2m
            ...
            enddo
          enddo
          if (k.eq.n3/2)write(42,*)myid,((xr(i,j),i=2,n1),j=1,n2m)
          call fft99(xr,work,trigx1,ifx1,1,m1+1,n1m,n2m,+1)
          if (k.eq.n3/2)write(62,*)myid,((xr(i,j),i=1,n1+1),j=1,n2m)
          do j=1,n2m
            dph(n1m,j,k)=xr(1,j)
            do i=1,n1mu
              is=i+1
