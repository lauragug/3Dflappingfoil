...
c
      ...
      character*4 dummy
      common/dim/n1glob,n1mglob,n1,n1m,n2,n2m,n3,n3m 
      common/d13/alx3
...
c
      ...
      include 'parade.f'
      implicit real*4(a-h,o-z)
c      dimension q1(m1,m2,m3),q2(m1,m2,m3),q3(m1,m2,m3)   !laura
...
c      dimension pr(m1,m2,m3),prc(m1,m2,m3)        !laura
     ...
     %                angle,omegak,osci,phi,ampp,sigma
      ...
      call indic
      call outpf(time,vo1,vo2,vo3,voc1,voc2,voc3,pr,densc,
     %                               vora,eig1)
...
c     real*8 dq1(m1,m2,m3), dq2(m1,m2,m3), dq3(m1,m2,m3),
c    %                      dpr(m1,m2,m3), ddens(m1,m2,m3)
c    %        ,add,dtime,aaa,prsub
C      dimension q1(m1,m2,m3), q2(m1,m2,m3), q3(m1,m2,m3),   !modifica laura
C     %                      pr(m1,m2,m3), dens(m1,m2,m3)
      dimension q1(m1,m2,m3), q2(m1,m2,m3), q3(m1,m2,m3),   
     %                      pr(m1,m2,m3), dens(m1,m2,m3)   
c      real*8 qq1(0:m1,m2,m3), qq2(0:m1,m2,m3), qq3(0:m1,m2,m3),   
c     %                      ppr(0:m1,m2,m3)
c      real*8 rtime
      dimension densre(129,m2,m3)
      character*70 namfi1,namfi2
      character*5 ipfi
      ...
      call system("ls *.dat")
      write(6,*) ' ---------------------------------'
      write(6,*) ' ENTER ROOT INPUT FILES '
      read(5,1000) namfi1
 1000 format(a70)
      ...
      do iproc = 0,numprocs-1
         ...
         write(ipfi,98)iproc
 98      format(i2.2)
         namfi2 = namfi1//'-'//ipfi//'.dat'
         write(6,201) namfi2
 201     format(10x,'leggo da ',a70)
         n3pp=(n3-1)/n3p+1           
         n2pp=(n2-1)/n2p+1          
         n1pp=(n1-1)/n1p+1  
         isx1 = 1 + iproc*int(n1mglob/numprocs)
         iex1 = isx1 + int(n1mglob/numprocs) - 1 

         open(59,file=namfi2,form='unformatted',status='unknown')
         ...
         read(59)   (((q1(i,j,k),i=1,n1,n1p),j=1,n2,n2p),k=1,n3,n3p), 
     %        (((q2(i,j,k),i=isx1,iex1,n1p),j=1,n2,n2p),k=1,n3,n3p),
     %        (((q3(i,j,k),i=isx1,iex1,n1p),j=1,n2,n2p),k=1,n3,n3p),
     %        (((pr(i,j,k),i=isx1,iex1,n1p),j=1,n2,n2p),k=1,n3,n3p)
         close(59)
...
c
 ...
 99   format(i5.5)
      ...
      do k=1,n3
         do j=1,n2
            do i=1,n1
               q1max = max(q1max,abs(voc1(i,j,k)))
               q2max = max(q2max,abs(voc2(i,j,k)))
               q3max = max(q3max,abs(voc3(i,j,k)))
               prmax = max(prmax,abs(prc(i,j,k)))
            end do
         end do
      end do
...
c     $                    - wx*(x3m(k)-x30)*cos(alphax) -
c     $                    wx*(x2m(j)-x20)*sin(alphax)
                     voc3ass(i,j,k) = voc2(i,j,k)*sin(alphax) +
     $                    voc3(i,j,k)*cos(alphax)
c     $                    - wx*(x3m(k)-x30)*sin(alphax) +
c     $                    wx*(x2m(j)-x20)*cos(alphax)
...
c     &  indgeo(5,mpuni,3),
     ...
     %,u0x,du0x,u0y,du0y,u0z,du0z,x10,x20,x30,hx,hy,uhy,duhy,ros
      ...
      common/emotion/ x3b_0,x2b_0,x1b_0,x3b_0_in,x2b_0_in,
     %                angle,omegak,osci,phi,ampp,sigma
c      real*8 q1(m1,m2,m3),q2(m1,m2,m3),q3(m1,m2,m3),  CHIEDERE FRANCESCO
c      dimension q1(m1,m2,m3),q2(m1,m2,m3),q3(m1,m2,m3),  
c     %          pr(m1,m2,m3),dens(m1,m2,m3)
      dimension q1(m1,m2,m3), q2(m1,m2,m3), q3(m1,m2,m3),   
     %                      pr(m1,m2,m3), dens(m1,m2,m3)
      dimension voc1(m1,m2,m3),voc2(m1,m2,m3),voc3(m1,m2,m3),
...
c 
      ...
      common/dim/n1,n1m,n2,n2m,n3,n3m       
      common/corrt/x1c(0:m1),x1m(0:m1)  
c      common/corrt/x1c(m1),x1m(m1)!laura
      common/cordvo/x2c(m2),x2m(m2),x3c(m3),x3m(m3)
      common/parcoo/r0,alx1
      common/d13/alx3
      common /metrr/  g2m(m2),g2c(m2)
...
c
      open(unit=98,file='radcor.outp',status='unknown')
      do j=1,n2
        write(98,*) j,float(j)/float(n2m),x2c(j),x2m(j),g2c(j),g2m(j)
      end do
      close(98)
      open(unit=78,file='axicor.outp',status='unknown')
      do k=1,n3
        write(78,*) k,float(k)/float(n3m),x3c(k),g3c(k),g3m(k)
      end do
      close(78)
      return                                                            
      end                                                               
...
c     open(61,file='extend.data',form='formatted',status='old')
      read(60,*) nz,ny,nx     
      n3=nz
      n2=ny
      n1=nx
...
c
      n1mh = n1m/2+1
      open(71,file='debugz.out',form='formatted',status='unknown')
      do n=1,npunz
        if(indgeo(2,n,1).eq.n1mh) write(71,71)
     %  indgeo(3,n,3),indgeo(3,n,2),distb(3,n)
      end do
      close(71)
 71   format(2(2x,i3),2x,f14.7)
      return
      end
c
      ...
      include 'parade.f'
      real*4 hx,hy,uhx,duhx,pi
      common /rotat/ wx,dwx,alphax,alphay,wy,dwy,wz,dwz
     %,u0x,du0x,u0y,du0y,u0z,du0z,x10,x20,x30,hx,hy,uhy,duhy,ros
      common /oscilinput/ uzero,amplit,thetamax
      common/tau/tau1,tau2

c     real*4 wx,dwx,wy,dwy,wz,dwz,u0x,du0x,u0y,du0y,
c    %       u0z,du0z,x10,x20,x30
c      write(*,*)'sono in rotation, time= ',tin
c    
...
c      dimension q1(m1,m2,m3),q2(m1,m2,m3),q3(m1,m2,m3)  !laura
      dimension q1(m1,m2,m3),q2(m1,m2,m3),q3(m1,m2,m3)  
c      dimension vora(m1,m2,m3) 
c      dimension voc1(m1,m2,m3),voc2(m1,m2,m3),voc3(m1,m2,m3)  
c      dimension voc1m(m2,m3)
c      dimension vmax(3),vocmax(4)
      dimension eig1(m1,m2,m3),eig2(m1,m2,m3)
      dimension tenso(3,3),defvel(3,3), rotvel(3,3)
c      common/circol/gamz,gamr,gamt
c      common/inslip/inslws,inslwn                                        
      common /nvort/ nvort
c      common/impuls/aipz,aipr,aipt
c      common/mimpul/ammz,ammr,ammt
      common /ametrz/  g3c(m3),g3m(m3),str3
...
!--------------------------------------------------------------------
     ...
     %                angle,omegak,osci,phi,ampp,sigma
       ...
       real x1,x2,y1,y2,z1,z2,small,pi,pitch
       real v11,v12,v13,v21,v22,v23,v31,v32,v33,sp,area,d32,d21,d13,
     $      sup_tot
       integer j,nb,i,nbtmp,ntmp,isg
       real*8 norb(17000,3)
       real*8 xyzb(17000,9)
       character(50)  namefile,string,string1
       real*4  time,x3b_0,x2b_0,x1b_0,x3b_0_in,x2b_0_in,
     %                angle,omegak,osci,phi,ampp,sigma
       real*4 xcen,ycen,angtime,radpun,angpun,xpun,ypun
!
       x1 = 10000.
       x2 =-10000.
       y1 = 10000.
