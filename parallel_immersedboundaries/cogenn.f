...
c                                                                       
      ...
      if (myid.eq.ii)then
        ...
        read(15,301) dummy
        read(15,*) n1glob,n2,n3,nsst,nwrit,nread
        read(15,301) dummy
        read(15,*) n1p,n2p,n3p
        read(15,301) dummy
...
301     format(a4)                
      ...
      tau3 = tau1+tau2
c
      if (myid.eq.0)call openfi    !PARALL
c
      pi=2.*asin(1.)                                                    
      tfini=dt*ntst                                                     
      tfin=tfini                                                        
      n1mglob=n1glob-1                                                          
c      if(n1glob.eq.1) n1mglob = 1
      n1m = n1mglob/numprocs
      n1 = n1m + 1
      n2m=n2-1                                                          
      n3m=n3-1
      n3mh=n3m/2+1
      if(n1mglob.eq.1) then
        iaxsy=0
      else
        iaxsy=1
      endif
...
c
   ...
   10 continue
c
c     master prints on output files
c
      if (myid.eq.0)then
         if(nsst.gt.1) then   
...
c  112 format(//,20x,'P A P E R O ',//,10x,
c     % 'Perturbazioni Azimutali in PEntola ROtante',//,5x,
c     % 'Cilindro:  H0=',f4.2,' R0=',f4.2,' H1=',e10.3,' R1=1.')
...
c***********************************************************************
      ...
      read(46,'(a)')filso
      open(32,file=filth)
c      open(34,file=filba)
c      open(35,file='filbaper.out')
c      open(39,file=filen)
c      open(40,file='errcal.out')
      open(65,file='forces.out',form='formatted')
c      open(66,file='coefor.out',form='formatted')
      open(67,file='move.out',form='formatted')
c      open(88,file='drag.dat',form='formatted')
c      open(95,file='hsep.dat',form='formatted')
c      open(97,file=filte)
c     if(nson.ne.0) then
c      nul=50
...
c***********************************************************************
c
c      subroutine cfl(cflm,q1,q2,q3)            ! PARALL                         
      subroutine cfl(cflm,comm)                                     
      include'param.f'
...
c                                                                       
     ...
     %               +abs((q3(i,j,k)+q3(i,j,kp))*0.5*udx3m(k)))    
                cflm = max(cflm,qcf)
  7   continue
      call MPI_ALLREDUCE(cflm,cflmall,1,MPI_DOUBLE_PRECISION,
     $     MPI_MAX,comm,ierr)
      cflm = cflmall
...
c***********************************************************************
c
C      subroutine divgck(qmax,qtot,q1,q2,q3) ! residuo !
      subroutine divgck(qmax,qtot,comm) ! residuo ! PARALL
      include'param.f'
      include "mpif.h"       !PARALL         
      integer comm,ierr    !PARALL
c                      
...
c     qqmax=qmax!PARALL
      call MPI_ALLREDUCE(qmax,qmaxall,1,MPI_DOUBLE_PRECISION,
     $     MPI_MAX,comm,ierr)
      qqmax=qmaxall
      qmax = qqmax
      call MPI_ALLREDUCE(qtot,qtotall,1,MPI_DOUBLE_PRECISION,
     $     MPI_SUM,comm,ierr)
c     qtot=qtot*dvol!PARALL
      qtot=qtotall*dvol
...
c***********************************************************************
      ...
      dx1=(x1c(n1glob)-x1c(1))/float(n1mglob)       !PARALL               
      dx2=1./float(n2m)                                                 
      dx3=1./float(n3m)                                               
      if (myid.eq.0)write(6,*)'dx1, dx2, dx3  ',dx1,dx2,dx3                           
      dx1=1./dx1                                                        
      dx2=1./dx2                                                        
      dx3=1./dx3                                                        
      dx1q=dx1*dx1                                                      
...
c                                                                       
c***********************************************************************
c      subroutine vmaxv(time,q1,q2,q3)        !PARALL               
      subroutine vmaxv(time,comm) 
      include'param.f'
      include "mpif.h"
      integer comm, ierr
c
 ...
 312  continue 
      ...
      call MPI_ALLREDUCE(vmax(1),vmaxall(1),1,MPI_DOUBLE_PRECISION,
     $     MPI_MAX,comm,ierr)
      call MPI_ALLREDUCE(vmax(2),vmaxall(2),1,MPI_DOUBLE_PRECISION,
     $     MPI_MAX,comm,ierr)
      call MPI_ALLREDUCE(vmax(3),vmaxall(3),1,MPI_DOUBLE_PRECISION,
     $     MPI_MAX,comm,ierr)
...
c***********************************************************************
c
c      subroutine continua(time,q1,q2,q3,pr)   !PARALL          
      subroutine continua(time,comm)             
      include'param.f'
      include "mpif.h"
   ...
   98 format(i2.2)
      ...
      filcnw='continua'//ipfi2//'.dat'
c     filcnw = 'continua.dat'
...
c***********************************************************************
c
c      subroutine inirea(ntii,time,q1,q2,q3,pr)    ! PARALL
      subroutine inirea(ntii,time,comm)
      include'param.f'
      include "mpif.h"
...
c***********************************************************************
c
c      subroutine initia(q1,ru1,q2,ru2,q3,ru3,pr,dq)
      subroutine initia      !PARALL
      include'param.f'
...
c            q3(i,j,k)=uzero*cos(thetamax*pi/180) 
            ...
            qcap(i,j,k)=0.
            dph(i,j,k)=0.
            st(i,j,k,1)=0.      !!!!!!!!VERIFICARE VIA VIA QUESTE MATRICI!!!!!
            st(i,j,k,2)=0.
            st(i,j,k,3)=0.
            st(i,j,k,4)=0.
            st(i,j,k,5)=0.
...
c***********************************************************************
c
      subroutine outh(time,cflm,voamax,vormax,vozmax)
      include'param.f'
...
c                                                                      *
c***********************************************************************
c      subroutine wson(time,q1,q2,q3,pr)
c      include 'param.f'
c      character*3 ipfis
c      character*70 namson
c      REAL q1(m1,m2,m3),q2(m1,m2,m3),q3(m1,m2,m3)
c      REAL   pr(m1,m2,m3)
cc
cc     This is to refer the pressure to a fixed value
cc
c        ice = n1m/2+1
c        jce = n2m/2+1
c        prsub = pr(ice,jce,2)
cc
c        nul=50
c      do i = 1,nson
c        ir = mason(i,1)
...
c        q2sur = q2(ir,jr,kr)
cc
cc       azimuthal vorticity
cc
c              dq2x3=(q2(ir,jr,kr)-q2(ir,jr,krm))*udx3
c              dq3x2=(q3(ir,jr,kr)-q3(ir,jrm,kr))*udx2
c              voraz=dq2x3-dq3x2
cc
cc       radial vorticity
cc
c              dq3x1=(q3(ir,jr,kr)-q3(irm,jr,kr))*dx1*iaxsy
c              dq1x3=(q1(ir,jr,kr)-q1(ir,jr,krm))*dx3/g3c(kr)
c              vorr=dq3x1-dq1x3
cc
cc       axial vorticity
cc
c              dq1x2=(q1(ir,jr,kr)-q1(ir,jrm,kr))
c     %           *udx2
c              dq2x1=(q2(ir,jr,kr)-q2(irm,jr,kr))*dx1*iaxsy
c              vorz=(dq1x2-dq2x1)
c        write(ipfis,99) nul
c   99 format(i3.3)
c      namson='son'//ipfis//'.out'
c      open(99,file=namson,form='formatted',status='unknown')
c      if(ntime.eq.1) go to 30
c 20   read(99,100,end=10) aa,aa,aa,aa,aa,aa,aa,aa
c      go to 20
c 10   continue
c      backspace(99)
c 30     write(99,100)
c     %   time,q1(ir,jr,kr),q2sur,q3(ir,jr,kr),
c     %   pr(ir,jr,kr)-prsub,voraz,vorr,vorz
c      close(99)
c        nul=nul+1
c      end do
c 100  format(2x,8(e12.5,2x))
cc
c      return
c      end     
c***********************************************************************
...
c***********************************************************************
c
c      subroutine vort(voamax,vormax,vozmax,q1,q2,q3)        !PARALL            
      subroutine vort(voamax,vormax,vozmax,comm)                    
      include'param.f'
      include "mpif.h"
      integer comm,ierr
c                                                                       
      voamax=0.   ! vorticita azimutale !   
      vmmx=0.                 
      do 2 kc=1,n3m           
        km=kmv(kc)           
        do 2 jc=2,n2m       
    ...
    2 continue                  
c     
      call MPI_ALLREDUCE(voamax,voamax_all,1,MPI_DOUBLE_PRECISION,
     $     MPI_MAX,comm,ierr)
      voamax = voamax_all
c        
      vormax=0.      ! vorticita radiale !  
      do 4 kc=1,n3m                         
        km=kmv(kc)                         
        do 4 jc=1,n2m                     
        do 4 ic=1,n1m                    
    ...
    4 continue                   
c
      call MPI_ALLREDUCE(vormax,vormax_all,1,MPI_DOUBLE_PRECISION,
     $     MPI_MAX,comm,ierr)
      vormax = vormax_all
c                               
      vozmax=0.     ! vorticita verticale !
      do 3 kc=2,n3m                         
        do 3 jc=2,n2m                         
          jm=jmv(jc)                           
          udx2 = dx2/g2c(jc)                               
    ...
    3 continue     
c
      call MPI_ALLREDUCE(vozmax,vozmax_all,1,MPI_DOUBLE_PRECISION,
     $     MPI_MAX,comm,ierr)
      vozmax = vozmax_all
c         
      return               
...
c       subroutine errcal(q1,q2,q3,time)
c       include 'param.f'
c       REAL q1(m1,m2,m3),q2(m1,m2,m3),q3(m1,m2,m3)
c
c       umax = 0.
...
cc         j=indgeos(l,1)
cc         k=indgeos(l,2)
c          usquare=(q1(i,j,k))**2+q2(i,j,k)**2+(q3(i,j,k))**2
c          umod=sqrt(usquare)
c          umax=max(umax,umod)
c          uhat1=uhat1+usquare*g2m(j)*g3m(k)/(dx2*dx3)
c          suptot = suptot + g2m(j)*g3m(k)/(dx2*dx3)
c         end do
c        end do
c       uhat=sqrt(uhat1/float(n1m)/suptot)
c       umaxt = 0.
c       do k=1,n3m
c         do j=2,n2m
c           do i=1,n1m
c             usquare=(q1(i,j,k))**2+q2(i,j,k)**2+(q3(i,j,k))**2
c             umod=sqrt(usquare)
c             umaxt=max(umaxt,umod)
c           end do
c         end do
c       end do
c       write(40,*) time, umax, uhat,umaxt
c        umax = 0.
c        do i=1,n1m
c         do l=1,npuns
cc         j=indgeos(l,1)
...
c         end do
c        end do
c        write(6,*) ' UMAX = ',umax,' at J = ',jvm,' K = ',kvm
c        write(6,*) ' rms U_b = ',uhat,' UMAX_t = ',umaxt
c       return
c       end 
c
c***********************************************************************
c      subroutine tripvmy(n1i,n1f,n2i,n2f,n3i,n3f,nn1,nn2,iequa,q1)                                    
c      include'param.f'
cc                                                                       
cc     vectorized for right hand side and coefficients                   
cc                                                                       
c      dimension fn(m2),p(m2)                                            
c      REAL q1(m1,m2,m3)
c      ia = n1i + 1                                                       
c      ii = n1i + n1f                                                      
c      do k=n3i,n3f
c      kc = k
cc
cc   COEFFICIENTS FOR TRIDIAGONAL INVERSION
cc
c
cc
cc   Q_1
cc
c      if(iequa.eq.1) then
...
c            im = imv(ic)
c            ip = ipv(ic)
c            api(ic,jc)=-ugmmm *(1.-forclo(ic,jc,kc))*visc(kc)
c            ami(ic,jc)=-ugmmm *(1.-forclo(ic,jc,kc))*visc(kc)
c            aci(ic,jc)=1.+ugmmm*2.*(1.-forclo(ic,jc,kc))*visc(kc)
c         end do
c       end do
c       end if
cc
...
cc  RHO
cc
c      if(iequa.eq.4) then
c      betadx=0.5*dx1q*al*dt/pec
c      betanl=dt*dx1*al*0.25
c      do jc=1,n2m
c        ugmmm=betadx
c        do ic=1,n1m
c            im = imv(ic)
c            ip = ipv(ic)
c            api(ic,jc)=-ugmmm*visc(kc)
c            ami(ic,jc)=-ugmmm*visc(kc)
c            aci(ic,jc)=1.+ugmmm*2.*visc(kc)
c         end do
c       end do
c       end if
cc
cc  THE INVERSION STARTS
cc
c      do 20 j=n2i,n2f                                                     
c        q(j,n1i) = -api(n1i,j)/aci(n1i,j)                                        
c        s(j,n1i) = -ami(n1i,j)/aci(n1i,j)                                        
c        iaddf = n1f+(j-1)*nn1+(k-1)*nn1*nn2
c        iaddi = n1i+(j-1)*nn1+(k-1)*nn1*nn2
c        fn(j) = rhs(iaddf)                                                   
c        rhs(iaddi) = rhs(iaddi)/aci(n1i,j)                                         
c   20 continue                                                          
cc                                                                       
cc     forward elimination sweep                                         
cc                                                                       
c      do 10 i=ia,n1f                              
c        do 21 j=n2i,n2f                             
c          iadd = i+(j-1)*nn1+(k-1)*nn1*nn2
c          iaddm = i-1+(j-1)*nn1+(k-1)*nn1*nn2
c          p(j) =1./( aci(i,j) + ami(i,j)*q(j,i-1))
c          q(j,i) = - api(i,j)*p(j)                     
c          s(j,i) = - ami(i,j)*s(j,i-1)*p(j)                                   
c          rhs(iadd) = ( rhs(iadd) - ami(i,j)*rhs(iaddm))*p(j)
c   21   continue          
c   10 continue             
cc                                                                       
cc     backward pass                                                     
...
c        do 23 j=n2i,n2f   
c          iadd = i+(j-1)*nn1+(k-1)*nn1*nn2
c          s(j,i) = s(j,i) + q(j,i)*s(j,i+1)        
c          fei(j,i) = rhs(iadd) + q(j,i)*fei(j,i+1)  
c   23   continue                    
c   11 continue                     
c      do 24 j=n2i,n2f               
c        iaddf = n1f+(j-1)*nn1+(k-1)*nn1*nn2
c        rhs(iaddf)=(fn(j)-api(i,j)*fei(j,n1i) - 
c     %      ami(i,j)*fei(j,n1f-1))/(api(i,j)*s(j,n1i) +
c     %      ami(i,j)*s(j,n1f-1)+aci(i,j))          
c   24 continue          
cc                                                                       
cc     backward elimination pass                                         
cc                                                                       
c      do 12 l=ia,n1f         
c        i = ii -l          
c        do 25 j=n2i,n2f     
c          iadd = i+(j-1)*nn1+(k-1)*nn1*nn2
c          iaddf = n1f+(j-1)*nn1+(k-1)*nn1*nn2
c          rhs(iadd) = rhs(iaddf)*s(j,i) + fei(j,i)  
c   25   continue                               
c   12 continue                                
c      end do
c      return                                 
...
c
c      open(unit=98,file='radcor.out',status='unknown')
c      do j=1,n2
c        write(98,348) j,x2c(j),x2m(j),g2c(j),g2m(j)
c      end do
c      close(98)
c      open(unit=78,file='axicor.out',status='unknown')
c      do k=1,n3
c        write(78,348) k,x3c(k),x3m(k),g3c(k),g3m(k)
c      end do
c      close(78)
...
c***********************************************************************
      ...
      do k=1,n3                                                  
        do j=1,n2                                                    
c          do i=1,n1                COMMENTATO PARALL                
          do i=0,n1                                                   
            dq(i,j,k) = 0.
          end do
        end do
      end do
      if (myid.eq.numprocs/2)then
         ice = n1m/2+1
         jce = n2m/2+1
         prsub = pr(1,jce,2)
      endif
      call MPI_BCAST(prsub,1,MPI_DOUBLE_PRECISION,numprocs/2,comm,ierr)
      do k=1,n3m
         do j=1,n2m                                                   
            do i=0,n1                                                  
   ...
   99 format(i5.5)
   98 format(i2.2)
      namfi3='coge'//ipfi1//'-'//ipfi2//'.dat'
      if (myid.eq.0)write(6,201)time,namfi3
     ...
     &"dVx","dVy","dVz"'
      ...
      do i=1,n1
        ...
        do j=1,n2
          write(99,*)
     $           x3c(n3),x2c(j),x1c(igeo),
     $           qb3s(i,j),qb2s(i,j),qb1s(i,j),
     $           dqb3s(i,j),dqb2s(i,j),dqb1s(i,j)
        enddo
      enddo
      close(99)
c      
      namfi3='jcost'//ipfi1//'-'//ipfi2//'.dat'
      open(99,file=namfi3,form='formatted',status='unknown')
      write(99,*) 'VARIABLES = "X","Y","Z","Vx","Vy","Vz",
     $"Pres"'
      write(99,*) 'ZONE I = ',n3,', J = 1, K = ',n1, ', F=POINT'
      j = n2m/2
      do i = 1, n1
        igeo = isx1 + i - 1
        do k=1,n3
          write(99,*)
     $           x3c(k),x2c(j),x1c(igeo),
     $       q3(i,j,k),q2(i,j,k),q1(i,j,k),pr(i,j,k)
        enddo
      enddo
      close(99)
      namfi3='icost'//ipfi1//'-'//ipfi2//'.dat'
      open(99,file=namfi3,form='formatted',status='unknown')
      write(99,*) 'VARIABLES = "X","Y","Z","Vx","Vy","Vz",
     $"Pres"'
...
c      if(iles.ne.0) then 
c        namfi3='visct'//ipfi//'.dat'
c        n1pp=(n1-1)/n1p+1                                                 
c        n2pp=(n2-1)/n2p+1                                                 
c        n3pp=(n3-1)/n3p+1                                                 
c        open(99,file=namfi3,form='unformatted')
c        rewind 99
c        write(99) n1pp,n2pp,n3pp ,1
c        write(99) 
c     %  ((((visct(i,j,k)),i=1,n1,n1p),j=1,n2,n2p),k=1,n3,n3p)
c        close(99)
c      end if
...
c 
c***********************************************************************
c      subroutine dralical(time,q1,q2,q3,pr)   ! PARALL
      subroutine dralical(time)
      include'param.f'
